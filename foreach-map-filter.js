...
with the value of the key with a name of "first" and the value of a key with the name of 
...
the array that has the same value as the second parameter or undefined if the value is not found
in the array.
...
function findInObj(arr, key, searchValue) {
    ...
    let onlyOnce = true;
    arr.forEach(function(item) {
        if(item[key] === searchValue && onlyOnce) {
            returnThis = item;
            onlyOnce = false;
        }
    });
...
the vowels (both uppercased and lowercased) removed. Every character in the new string should be
lowercased.
...
function removeVowels(str) {
    ...
    const arr = str.split('');
    let finalString = "";
    let makeString = arr.filter(function(item) {
        return !vowels.includes(item);
    });
    makeString.forEach(function(charcter) {
...
the odd numbers doubled (HINT - you can use map and filter to double and then filter the odd numbers).
